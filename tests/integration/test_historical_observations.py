...
class TestHistoricalObservationsEndpoints:
    ...
    async def test_get_historical_data_for_station(self, client):
        ...
        response = client.get(
            "/v3/historical/TEST001?start_date=2023-01-01&end_date=2023-01-02"
        )
        assert response.status_code == 200
        data = response.json()
        assert data["id"] == "TEST001"
    ...
    async def test_get_historical_data_invalid_date_range(self, client):
        ...
        response = client.get(
            "/v3/historical/TEST001?start_date=2024-01-01&end_date=2024-01-02"
        )
        assert response.status_code == 404
        data = response.json()
        assert "detail" in data
    ...
    async def test_get_historical_data_invalid_date_format(self, client):
        ...
        response = client.get(
            "/v3/historical/TEST001?start_date=invalid-date&end_date=2023-01-02"
        )
